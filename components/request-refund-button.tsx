...
/**
 * Shopper-initiated refund request.
 *
 * Shown on Paid orders within the refund window. Calls
 * POST /api/account/orders/[id]/refund, which refunds the full remaining
 * captured amount. The merchant DCW signs server-side; no wallet action
 * required from the shopper.
 */
export function RequestRefundButton({ orderId }: { orderId: string }) {
  ...
  async function handleRefund() {
    ...
    setPhase("done");
    toast.success("Refund initiated - funds on their way back", {
      description: txHash ? (
        <a
  ...
  return (
    ...
    >
      ...
      {phase === "processing" ? "Requesting refund..." : "Request refund"}
    </button>
  );
}
